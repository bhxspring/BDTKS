...
for s=1:length(strings)
    ...
    best_acc = 0;
    for localErr = 10.^(-6:3)
        tr = acceleratedTwoMeansTreeModel_pcaInitialized_new(train_data(train,:),train_class(train),numDim,localErr);
        [~,label] = multivariateBinaryTreePrediction( train_data(val,:),tr,classValue );
    ...
    tr = acceleratedTwoMeansTreeModel_pcaInitialized_new(train_data,train_class,numDim,best_localErr);
    t_temp = clock;
    [~,label] = multivariateBinaryTreePrediction( test_data,tr,classValue );
    t_end = clock;
